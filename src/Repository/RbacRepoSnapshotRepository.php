...
namespace BlackCat\Database\Packages\RbacRepoSnapshots\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\RbacRepoSnapshots\Definitions;
use BlackCat\Database\Packages\RbacRepoSnapshots\Criteria;
use BlackCat\Database\Packages\RbacRepoSnapshots\Dto\RbacRepoSnapshotDto as Dto;
use BlackCat\Database\Packages\RbacRepoSnapshots\Mapper\RbacRepoSnapshotDtoMapper as RowMapper;
...
final class RbacRepoSnapshotRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\RbacRepoSnapshots\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'repo_id', 'commit_id' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\RbacRepoSnapshots\Definitions::table());
            return $bulk->upsertMany($rows, [ 'repo_id', 'commit_id' ], [ 'taken_at', 'metadata' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\RbacRepoSnapshots\Dto\RbacRepoSnapshotDto|null */
    public function getByRepoIdAndCommitId(int $repoId, string $commitId, bool $asDto = false): array|\BlackCat\Database\Packages\RbacRepoSnapshots\Dto\RbacRepoSnapshotDto|null {
        $row = $this->getByUnique([ 'repo_id' => $repoId, 'commit_id' => $commitId ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\RbacRepoSnapshots\Mapper\RbacRepoSnapshotDtoMapper::fromRow($row);
    }
    public function existsByRepoIdAndCommitId(int $repoId, string $commitId): bool {
        $where = 't.' . Ident::q($this->db, 'repo_id') . ' = :uniq_repo_id' . ' AND ' . 't.' . Ident::q($this->db, 'commit_id') . ' = :uniq_commit_id';
        return $this->exists($where, [ 'uniq_repo_id' => $repoId, 'uniq_commit_id' => $commitId ]);
    }
    /** @return int|string|null */
    public function getIdByRepoIdAndCommitId(int $repoId, string $commitId) {
        $row = $this->getByRepoIdAndCommitId($repoId, $commitId);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\RbacRepoSnapshots\Dto\RbacRepoSnapshotDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\RbacRepoSnapshots\Dto\RbacRepoSnapshotDto|null {
