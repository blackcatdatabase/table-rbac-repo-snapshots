...
use BlackCat\Database\Packages\RbacRepoSnapshots\Repository\RbacRepoSnapshotRepositoryInterface;
...
class RbacRepoSnapshotRepository implements RbacRepoSnapshotRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'repo_id', 'commit_id' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\RbacRepoSnapshots\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'taken_at', 'metadata' ]);
          return count($rows);
        }
    ...
    public function getByRepoIdAndCommitId(int $repoId, string $commitId, bool $asDto = false): array|\BlackCat\Database\Packages\RbacRepoSnapshots\Dto\RbacRepoSnapshotDto|null {
        return $this->getByUnique([ 'repo_id' => $repoId, 'commit_id' => $commitId ], $asDto);
    }
    ...
    public function getIdByRepoIdAndCommitId(int $repoId, string $commitId) {
        $row = $this->getByRepoIdAndCommitId($repoId, $commitId, false);
