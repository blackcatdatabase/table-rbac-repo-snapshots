...
namespace BlackCat\Database\Packages\RbacRepoSnapshots\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\RbacRepoSnapshots\Dto\RbacRepoSnapshotDto;
use BlackCat\Database\Packages\RbacRepoSnapshots\Mapper\RbacRepoSnapshotDtoMapper;
 ...
 */
final class RbacRepoSnapshotsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private RbacRepoSnapshotRepository $rbacRepoSnapshotRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'rbac_repo_snapshots' => $this->rbacRepoSnapshotRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\RbacRepoSnapshots\ModuleException("Record {$idStr} not found for locking.");
            }
